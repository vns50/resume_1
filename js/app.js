...
const textarea = document.querySelector('.form-page__textarea');
const valueSpan = document.querySelector('.form-page__value span');
...
if (isMobile.any()) {
	...
	searchHeaderIcon.addEventListener("click", function (e) {
		searchHeaderForm.classList.add('_active');
		searchHeaderIcon.classList.add('_hide');
	});
	...
	document.addEventListener("click", function (e) {
		if (!e.target.closest('.search-header__form') && !e.target.classList.contains('search-header__icon')) {
			...
			searchHeaderIcon.classList.remove('_hide');
		}
	});
	document.addEventListener("keydown", function (e) {
		if (e.code === 'Escape') {
...
if (itemsList) {
	const itemsListShow = itemsList.dataset.show;
	const itemsMore = document.querySelector('.items__more');
	...
	function setHeight(update = false) {
		const itemsListItems = document.querySelectorAll('.items__list li');
		const windowWidth = getClientWidth();
		if (update) {
			startHeight = 0;
			allHeight = 0;
		}
		itemsListItems.forEach((item, index) => {
			allHeight += item.offsetHeight;
			if (index < itemsListShow) {
	...
	itemsMore.addEventListener("click", function (e) {
		...
		if (!itemsMore.classList.contains('_active')) {
			itemsList.style.height = `${startHeight}px`;
		} else {
			itemsList.style.height = `${allHeight}px`;
			itemsList.addEventListener("transitionend", setHeight);
		}
		e.preventDefault();
	});
}
...
let priceItem = document.querySelector('.price__item');
let textFrom = "от";
let textTo = "до";
if (price) {
	...
	const priceValueFrom = document.querySelector('.price__value_from');
	const priceValueTo = document.querySelector('.price__value_to');

	priceValueFrom.addEventListener('change', setPriceValues);
	priceValueTo.addEventListener('change', setPriceValues);

	function setPriceValues() {
		let priceStartValue;
		let priceEndValue;
		if (priceValueFrom.value != '') {
			priceStartValue = priceValueFrom.value;
		}
		if (priceValueTo.value != '') {
			priceEndValue = priceValueTo.value;
		}
		priceItem.noUiSlider.set([priceStartValue, priceEndValue]);
	}

	priceItem.noUiSlider.on('update', function (values, handle) {
		const value = values[handle];
		const priceInput = handle === 0 ? priceValueFrom : priceValueTo;
		if (value) {
			priceInput.value = Math.round(value);
		}
	});
}

//========================================================================================================================================================
const dItems = document.querySelectorAll('.digi__item');
if (dItems.length > 0) {
...
const player = document.querySelector('#playerItem');
const playerPlay = document.querySelector('.player__play');
const playerStop = document.querySelector('.player__stop');

playerPlay.addEventListener("click", function (e) {
	player.play();
});
playerStop.addEventListener("click", function (e) {
	player.pause();
});
...
data-prlx-m="коэффициент_х, коэффициент_у, направление_относительно_Х, направление_относительно_Y, скорость-анимации", где:
1) коэффициент - 10px движения элемента = 1px движения мыши; (10 = движение мышью, значение больше - меньше процент сдвига);
2) направление вдоль оси - 
		true - по оси,
		false - против оси;
3) скорость-анимации - с какой скоростью движется элемент относительно мыши (1000 = движение мышью, больше значение - больше скорость)
...
if (paralaxMouse.length > 0) {
	paralaxMouse.forEach(el => {
		...
		// Принимаю параметры в переменную и делаю массив
		// [0] - коэффициент по Х
		// [1] - коэффициент по Y
		// [2] - по/против оси Х
		// [3] - по/против оси Y
		...
		let parameters = ['100', '100', 'true', 'true', '50'];

		if (el.dataset.prlxM) {
			parameters = el.dataset.prlxM.split(',');
		}

		directionX = parameters[2] == 'true' ? '1' : '-1';
		directionY = parameters[3] == 'true' ? '1' : '-1';
		...
		function setMouseParallaxStyle() {
			...
			positionX = positionX + (distX * parameters[4] / 1000);
			positionY = positionY + (distY * parameters[4] / 1000);
			el.style.cssText = `transform: translate(${directionX * positionX / (parameters[0] / 10)}%,${directionY * positionY / (parameters[1] / 10)}%);`;
